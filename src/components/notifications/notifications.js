...
export function showNotification(textOfNotification, type, someCallback){
  ...
  const newNotification = createNewElement("div", ["notification", "panel", "confirm"]);
  const newTextOfNotification = createNewElement("div", "text-of-notification");
  const newCloseNotificationButton = createNewElement("button", ["small-button", "red-button", "close-notification-button"]);
  ...
  if (!type) {
    ...
    const yesButton = createNewElement("button", ["big-button", "answer-button"]);
    const noButton = createNewElement("button", ["big-button","answer-button"]);
    ...
    newCloseNotificationButton.addEventListener("click", function notificationCallback() {
      notificationWindow.remove()
    });
